module Backend
  class BIND < Base
    ...
    def deploy
      ...
      logger.debug { "remove zones directory" }
      dst_zones_dir.rmtree

      logger.debug { "copy generated files" }
    ...
    def generate!
      ...
      @tmp_named_conf.open("w") do |f|
        src_zones_files.each do |file|
          ...
          logger.debug { "generating zone #{domain}" }
    ...
    def generate_zone(file, domain)
      ...
      if old_file.exist?
        logger.debug { "found an already deployed version" }
        ...
        if new_output != old_output
          logger.debug { "generating new serial" }
        ...
        logger.debug { "zone has not existed before" }
        mark_changed(domain, :created)
        new_zonefile.new_serial
        new_output = new_zonefile.output
      end

      logger.debug { "writing new zonefile" }
